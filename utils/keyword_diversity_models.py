...
def calculate_entropy_targets(
    ...
    *,
    ...
    # Ensure numeric clicks (Google Ads exports sometimes come in as strings)
    df[clicks_col] = pd.to_numeric(df[clicks_col], errors="coerce").fillna(0.0)
...
def calculate_hhi_targets(
    ...
    *,
    ...
    # Ensure numeric conv value (Google Ads exports sometimes come in as strings)
...
def default_artifact_paths(model_dir: Union[str, Path], embedding_method: str, n_components: int) -> DiversityModelArtifacts:
    ...
    *,
    ...
    mode:
      - "training": requires search_term_df; produces OOF preds for keywords with targets.
            - "prod": retrains models on all provided search_term_df and persists them for reuse.
    """
    ...
    if mode == "prod":
        # In prod, always retrain on the full (available) search-terms report.
        if search_term_df is None:
            raise ValueError("prod mode requires search_term_df to retrain diversity models")
    ...
    if entropy_model is None or hhi_model is None:
        ...
        # In training mode, we may optionally restrict target calculation to phrase/broad only
        # to avoid instability from exact matches. In prod mode we use all available data.
        if mode == "training" and 'Match type' in search_term_df.columns:
        ...
        if mode == "training" and "Match type" in main_df.columns:
            # Keep parity with the search_term_df restriction above.
