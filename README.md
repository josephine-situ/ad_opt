...
## Steps
1. Get reports from Google Ads (Search keyword - raw input to models.csv (saved), Search keyword - search terms (saved, use conv > 0), Purchase report (if using max-purch), and Location report (only if using max-conv from Audiences, keywords and content > Locations)). Get Semrush keywords (Semrush > SEO > Keyword Magic Tool).
