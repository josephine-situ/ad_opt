"""
Script to query the change_event table via search_stream over a configurable lookback period.

The change_event resource describes granular changes made to Google Ads resources
through the UI or API. Data is available for up to 30 days in the past.
See: https://developers.google.com/google-ads/api/fields/v24/change_event
"""
...
def query_change_events(
    ...
    """Query the change_event table for changes within the lookback period.

    Args:
        google_ads_client: Authenticated GoogleAdsClient instance.
        customer_id: Google Ads customer ID (digits only, no dashes).
        lookback_days: Number of days to look back from end_datetime.
        end_datetime: End of the query window. Defaults to now (UTC).

    Returns:
        List of dicts, one per change event row.
    """
...
def print_results(rows: list[dict[str, Any]]) -> None:
    """Print change event rows as TSV to stdout."""
    fieldnames = [
        "resource_name",
        "change_date_time",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Query the Google Ads change_event table over a configurable lookback period."
    )
    parser.add_argument(
        "--google-ads-yaml",
        type=str,
    ...
    print_results(rows)
    print(
        f"Queried {args.lookback_days} day(s) of change events ({len(rows)} total).",
    )
